...
##' 
##' @description Returns a file containing GWAS p-values for a region in bed format.
...
##' @return creates a bed file with -log10 of corrected p-value ("Pc1df") 
##' @examples
##'  \dontrun{
##'  chr <- 36
##'  range <- c(14e6,24e6)
...
##' @export gwaa2bed
gwaa2bed <- function(chr, range, gwas, fname="output.bed") {
  ...
  for (i in tmp) {
    ...
    t <- paste("chr",chr," ", pos-1, " ", pos, " ", pval, sep="")
