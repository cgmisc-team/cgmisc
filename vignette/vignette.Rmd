...
author: Marcin Kierczak, Jagoda Jabłońska, Simon Forsberg, Matteo Bianchi, Katarina
  Tengvall, Mats Pettersson, Veronika Scholz, Jennifer Meadows, Patric Jern, Orjan Carlborg, Kerstin
  Lindblad-Toh
date: "30 April 2015"
output:
  pdf_document:
...
pac <- pop.allele.counts(data = data.qc1[ ,data.qc1@gtdata@chromosome == 28], 
                         pops = pop, 
                         progress=F)
...
summary(mm, top = 1)
```
We see that the top-associated marker is **BICF2S2365880** and its position is **38256927bp**. We will zoom in on a 2 Mbp region centered on the marker using ```plot.manhattan.ld```
The aforementioned function produces a standard so-called Manhattan plot with color-coded linkage disequilibrium (LD) to a specific reference marker measured by r2. Given genotyping data (as GenABEL gwaa.data-class object) and GWAS result in the form of p-values vector together with genetic coordinates of a region to be plotted (up to entire chromosome),  plot.manhattan.ld produces a plot with genomic coordinates on the x-axis and -log10(p-value) on the Y axis. Linkage disequilibrium to a reference marker is represented by specific colours. By default, colours represent 5 discrete intervals of r2: [1.0, 0.8[; [0.8, 0.6[; [0.6, 0.4[; [0.4, 0.2[; [0.2, 0[, where: [a, b[ := {x: a >= x > b}. These intervals can be altered using function parameters.
...
```{r, echo=TRUE, results='show', message=TRUE, warning=FALSE, eval=T}
fpath <- system.file('extdata', 'canis_familiaris.prot.bed', package = 'cgmisc')
...
```{r plot_qq, fig.align='center', dev='quartz_jpeg', eval=TRUE, echo=TRUE, results='hide', fig.width=580, fig.height=580}
