...
# Now, we can assign our individuals to the calculated clusters which represent subpopulations.
pop <- kclust$cluster
```
...
pac <- pop.allele.counts(data.qc1[,data.qc1@gtdata@chromosome==28], pop, progress=F)
plot.pac(data.qc1[,data.qc1@gtdata@chromosome==28], allele.cnt = pac, 
         plot.LD = T, legend.pos='topleft')
```
...
```{r, echo=TRUE, results='hide', message=FALSE, warning=FALSE, eval=T}
h2h <- polygenic(formula = ct ~ sex, data.qc1.gkin, 
                 data.qc1, llfun='polylik')
...
plot.manhattan.LD(data = data.qc1, gwas.result = mm, chr = 2, region = c(37256927,39256927), 
                  index.snp = "BICF2S2365880", legend.pos = "topleft")
```

```{r, echo=TRUE, results='show', message=TRUE, warning=FALSE, eval=T}
fpath <- system.file("extdata", "canis_familiaris.prot.bed", package="cgmisc")
plot.manhattan.genes(data = data.qc1, gwas.result = mm, chr = 2, 
                     region = c(37256927,39256927), 
                     index.snp = "BICF2S2365880", bed.path=fpath)
```
...
plot.LD.decay(data.qc1[,data.qc1@gtdata@chromosome==28])
```
...
```{r plot_qq, fig.align='center', dev='pdf', eval=TRUE, echo=TRUE, results='hide'}
# Perform permutation tests
h2h <- polygenic(formula = ct ~ sex, data.qc1.gkin, data.qc1, llfun='polylik')
result <- c()
h2h.tmp <- h2h
pb <- txtProgressBar(min=0, max=30, initial=0, style=3)
for (i in 1:30) {
  h2h.tmp$grresidualY <- sample(h2h$grresidualY)
  tmp <- qtscore(h2h.tmp$grresidualY, data.qc1, strata = pop, clambda = F)
  result <- rbind(result, sort(-log10(tmp@results$P1df)))
  setTxtProgressBar(pb,i,label = "Perm. test")
}
plot.qq(obs = mm[,'P1df'], emp = result, plot.emp = T, step = 100)
plot.qq(data = data.qc1, obs = mm[,'P1df'], emp = h2h, N = 10, plot.emp = T, step = 100)
```
